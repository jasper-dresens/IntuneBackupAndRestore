    ...
    foreach ($deviceCompliancePolicy in $deviceCompliancePolicies) {
        ...
        try {
            if ($restoreById) {
                ...
                $deviceCompliancePolicyObject = Get-DeviceManagement_DeviceCompliancePolicies | Get-MGGraphAllPages | Where-Object displayName -eq "$($deviceCompliancePolicy.BaseName)"
